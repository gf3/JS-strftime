...
if (typeof Date.prototype.strftime == 'undefined') {
  ...
  Date.prototype.strftime = (function(){
    ...
    function day_of_year(date) {
      return ((date.getTime() - cache['start_of_year'].getTime()) / day_in_ms + 1).toFixed();
    }
    
    // day_padded
    function day_padded(date) {
      return ('0' + day(date)).substr(-2);
    }
    ...
    function hour_24_padded(date) {
      return ('0' + hour_24(date)).substr(-2);
    }
    
    // hour_padded
    function hour_padded(date) {
      return ('0' + hour(date)).substr(-2);
    }
    ...
    function second(date) {
      return ('0' + (date.getSeconds() + 1)).substr(-2);
    }
    
    // time_zone_offset
    function time_zone_offset(date) {
      // TODO GIANNI: This is missing the leading zero
      return ((date.getTimezoneOffset() / 60 * 100) * -1) + '';
    }
    ...
    function year_abbr(date) {
      return year(date).substr(-2);
    }
    
    /*------------------------------ Main ------------------------------*/
    function strftime(format) {
      // TODO GIANNI: Update cached date to this year
      var match, output = format;
      
      while (match = regexp.exec(format)) {
        // console.log("match", match);
        // console.log("replacer", formats[match[1]](this));
        // console.log("RegExp", new RegExp(match[0], 'mg'));
