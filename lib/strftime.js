...
if (typeof Date.prototype.strftime == 'undefined') {
  ...
  Date.prototype.strftime = (function(){
    ...
    function doy(date){
      var m = date.getMonth()
      return date.getDate() + month_day_offsets[m] + (m > 1 && leap_year(date.getFullYear()) ? 1 : 0);
    }
    // day_of_year
    function day_of_year(date) {
      return doy(date).toString();
    }
    ...
    function week_number_from_day(date, day) {
      first_day = cache['start_of_year'].getDay();
      first_week_start = (7 - first_day + day) % 7;
      return ('0' + Math.ceil((doy(date) - first_week_start) / 7)).slice(-2);
    }

    // week_number_from_sunday
    function week_number_from_sunday(date) {
      return week_number_from_day(date, 0);
    }

    // week_number_from_monday
    function week_number_from_monday(date) {
      return week_number_from_day(date, 1);
    }
